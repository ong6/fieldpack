...
const products = ['deckforge', 'skillforge', 'proofpack'];
...
function doctor() {
  const checks = products.map(name => {
    ...
    if (!existsSync(path.join(location, '.git'))) throw new Error(`${name}: submodule missing. Clone with --recurse-submodules, or run git submodule update --init.`);
