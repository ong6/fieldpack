...
test('CLI help works through real and symlinked absolute paths without writing data', async () => { const run = promisify(execFile); const result = await run(process.execPath, ['/home/jun.ong/Sideproject/proofpack/server.js', '--help']); assert.match(result.stdout, /127\.0\.0\.1:4313/); assert.match(result.stdout, /npm test/); await assert.rejects(run(process.execPath, [path.join(ROOT, 'server.js'), '--unknown'])); });
